create or replace package body as_read_xlsx is
  --
  function read(
    p_xlsx     blob,
    ...
    t_one_cell     tp_one_cell := tp_one_cell( null, null, null, null, null, null, null, null, null, null );
    t_ndoc         dbms_xmldom.domdocument;
    sPrefix        varchar2( 3 ) := 'xl/';

    --
    function blob2node( p_blob blob )
      return dbms_xmldom.domnode is
      /*begin
        if p_blob is null or
           dbms_lob.getlength( p_blob ) = 0 then
          return null;
        end if;

        if not dbms_xmldom.isnull( t_ndoc ) then
    ...
    begin
      ...
      t_dest_offset  := 1;
      t_src_offset   := 1;
      dbms_lob.converttoclob( t_clob, p_blob, dbms_lob.lobmaxsize, t_dest_offset, t_src_offset, nls_charset_id( 'AL32UTF8' ), t_lang_context, t_warning );
    ...
    begin
      ...
      loop
        exit when t_ind < 1 or
                  dbms_lob.substr( p_zipped_blob, 4, t_ind ) = hextoraw( '504B0506' ); -- End of central directory signature
        t_ind  := t_ind - 1;
      end loop;
      ...
      for i in 1 .. blob2num( p_zipped_blob, 2, t_ind + 8 ) loop
        ...
        if p_file_name = utl_i18n.raw_to_char( dbms_lob.substr( p_zipped_blob, blob2num( p_zipped_blob, 2, t_hd_ind + 28 ), t_hd_ind + 46 ), t_encoding ) then
  ...
  begin
    ...
    t_nd                   := blob2node( get_file( p_xlsx, sPrefix || 'workbook.xml' ) );

    if dbms_xmldom.isnull( t_nd ) then
      if ( sRetry = 'T' ) then
        sPrefix  := null;
        t_nd     := blob2node( get_file( p_xlsx, sPrefix || 'workbook.xml' ) );
      end if;
    ...
    t_nd                   := blob2node( get_file( p_xlsx, sPrefix || 'styles.xml' ) );
    ...
    t_nd                   := blob2node( get_file( p_xlsx, sPrefix || 'sharedStrings.xml' ) );
    ...
    t_nd2                  := blob2node( get_file( p_xlsx, sPrefix || '_rels/workbook.xml.rels' ) );
    ...
    for i in 1 .. t_sheet_ids.count loop
      if ( p_sheets is null or
        ...
        t_nd                   := blob2node( get_file( p_xlsx, sPrefix || t_val ) );
          ...
          for j in 0 .. dbms_xmldom.getlength( t_nl2 ) - 1 loop
            ...
            if t_t in ('str', 'inlineStr', 'e') then
              ...
              t_nr      := to_number( t_val,
                                      case
                                        when instr( t_val, 'E' ) = 0 then translate( t_val, '.012345678,-+', 'D999999999' )
                                        else translate( substr( t_val, 1, instr( t_val, 'E' ) - 1 ), '.012345678,-+', 'D999999999' ) || 'EEEE'
                                      end,
                                      'NLS_NUMERIC_CHARACTERS=.,'
                                     );

              if t_s is not null and
            ...
            if p_cell is null or
               t_r = upper( p_cell ) then
              pipe row ( t_one_cell );
            end if;
          end loop;
        end loop;
